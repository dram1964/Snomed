...
while ( my $procedure = $procedures->next ) {
    if ( $debug <= 4 ) {
        print ++$procedure_count, ")", $double_quote,
          $procedure->current_procedure, $double_quote, "\n";
    }
...
sub find_matches {
    ...
    my @procedure_words =
      grep( !/$regex/, split /\s{1,}/, $procedure->current_procedure );
    my %procedure_words;
    ...
    while ( my $term = $terms->next ) {
        ...
        for my $field (@fields) {
            if ( $term->$field ) {
                ...
                my @match_words =
                  grep( !/$regex/, split /\s{1,5}/, $term->$field );
                my %match_words;
                ...
                if ( $debug <= 2 ) {
                    print join( ":", ( keys %{ $matches{$field} } ) ), "\n"
                      if ( $debug == 1 );
                    print scalar( keys %{ $matches{$field} } ),
                      " $field Words found\n";
                }
                ...
                if ( $debug <= 3 ) {
                    if ( $percent_match == 100 && $percent_miss == 0 ) {
                        print "Perfect match: ", $double_quote, $term->$field,
                          $double_quote, " to ",
                          $double_quote, $procedure->current_procedure,
                          $double_quote, "\n";
                    }
                    elsif ( $percent_match == 0 && $percent_miss >= 100 ) {
                        print "No match: ", $double_quote, $term->$field,
                          $double_quote, " to ",
                          $double_quote, $procedure->current_procedure,
                          $double_quote, "\n";
                    }
                    else {
                        print "$hit Matches: $miss Misses for ", $double_quote,
                          $term->$field, $double_quote,
                          " to ", $double_quote, $procedure->current_procedure,
                          $double_quote, "\n";
                    }
                }
                if ( $percent_match >= 70 ) {
                    print "Match Found: ", $double_quote, $term->$field,
                      $double_quote, " to ",
                      $double_quote, $procedure->current_procedure,
                      $double_quote, "\n";
                    $csv->print( $fh, $_ )
